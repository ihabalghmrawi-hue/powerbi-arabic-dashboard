...
export default function SignupPage() {
  ...
  async function handleSignup(e: React.FormEvent) {
    ...
    // Call our custom API (uses Supabase admin API — no email sent, no rate limit)
    ...
    // Account created — log in immediately (no email confirmation needed)
    const { error: loginErr } = await supabase.auth.signInWithPassword({ email, password })
    if (loginErr) {
      setError('تم إنشاء الحساب لكن فشل تسجيل الدخول التلقائي — جرّب تسجيل الدخول يدوياً')
      setLoading(false)
      return
    }
    ...
    <div style={styles.page}>
      <div style={styles.card}>
        ...
        <div style={styles.hint}>
          <strong>✅ بدون تأكيد بريد إلكتروني</strong><br />
          الحساب يُفعَّل فوراً — لا حاجة لتأكيد أي بريد إلكتروني.
        </div>
